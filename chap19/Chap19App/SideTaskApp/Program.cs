...
namespace SideTaskApp
{
    class SideTask
    {
        ...
        public void CountDown()
        {
            ...
            {
                Console.WriteLine($"임의종료 예외발생 : {ex.Message}");
                // Thread.ResetAbort();
            }
            catch (Exception ex)
            {
    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            th.IsBackground = false; // 메인 스레드와 백그라운드가 따로 종료된다. (이 스레드가 종료되기 전까지는 모든 프로세스가 끝나지 않음)
            // true인 경우, 백그라운드 스레드가 돌고있는 상황에도 메인 스레드가 끝나면 백그라운드 스레드도 같이 종료
